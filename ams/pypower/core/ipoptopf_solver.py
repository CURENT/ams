...
from scipy.sparse import issparse, tril, vstack, hstack, csr_matrix as c_sparse
...
from ams.pypower.util import sub2ind, feval
...
#        c = totcost(gencost[ipwl, :], PQ[ipwl])
        ...
        c = gencost.flatten('F')[sub2ind(np.shape(gencost), ipwl,
                                         pidx.cost['NCOST'] + 2 * gencost[ipwl, pidx.cost['NCOST']])]
...
#        x0[vv['i1']['y']:vv['iN']['y']) = c + 0.1 * abs(c)
    ...
    Cl2 = Cl[il, :]
    Cg = c_sparse((np.ones(ng), (gen[:, pidx.gen['GEN_BUS']], np.arange(ng))), (nb, ng))
    nz = nx - 2 * (nb + ng)
    nxtra = nx - 2 * nb
    if nz > 0:
        Js = vstack([
            hstack([Cb, Cb, Cg, c_sparse((nb, ng)), c_sparse((nb,  nz))]),
            hstack([Cb, Cb, c_sparse((nb, ng)), Cg, c_sparse((nb,  nz))]),
            hstack([Cl2, Cl2, c_sparse((nl2, 2 * ng)), c_sparse((nl2, nz))]),
            hstack([Cl2, Cl2, c_sparse((nl2, 2 * ng)), c_sparse((nl2, nz))])
        ], 'coo')
    else:
        Js = vstack([
            hstack([Cb, Cb, Cg, c_sparse((nb, ng))]),
            hstack([Cb, Cb, c_sparse((nb, ng)), Cg,]),
            hstack([Cl2, Cl2, c_sparse((nl2, 2 * ng)),]),
            hstack([Cl2, Cl2, c_sparse((nl2, 2 * ng)),])
        ], 'coo')
    ...
    Hs = tril(d2f + vstack([
        hstack([Cb,  Cb,  c_sparse((nb, nxtra))]),
        hstack([Cb,  Cb,  c_sparse((nb, nxtra))]),
        c_sparse((nxtra, nx))
    ]), format='coo')
...
#    options['ipopt'] = ipopt_options([], ppopt)
    ...
    f_br = branch[:, pidx.branch['F_BUS']].astype(int)
    t_br = branch[:, pidx.branch['T_BUS']].astype(int)
    ...
    muSt = np.zeros(nl)
    if len(il) > 0:
        muSf[il] = 2 * info['lmbda'][2 * nb + np.arange(nl2)] * branch[il, pidx.branch['RATE_A']] / baseMVA
        muSt[il] = 2 * info['lmbda'][2 * nb + nl2 + np.arange(nl2)] * branch[il, pidx.branch['RATE_A']] / baseMVA

    # update Lagrange multipliers
    bus[:, pidx.bus['MU_VMAX']] = info['zu'][vv['i1']['Vm']:vv['iN']['Vm']]
    bus[:, pidx.bus['MU_VMIN']] = info['zl'][vv['i1']['Vm']:vv['iN']['Vm']]
    gen[:, pidx.gen['MU_PMAX']] = info['zu'][vv['i1']['Pg']:vv['iN']['Pg']] / baseMVA
    gen[:, pidx.gen['MU_PMIN']] = info['zl'][vv['i1']['Pg']:vv['iN']['Pg']] / baseMVA
    gen[:, pidx.gen['MU_QMAX']] = info['zu'][vv['i1']['Qg']:vv['iN']['Qg']] / baseMVA
    gen[:, pidx.gen['MU_QMIN']] = info['zl'][vv['i1']['Qg']:vv['iN']['Qg']] / baseMVA
    bus[:, pidx.bus['LAM_P']] = info['lmbda'][nn['i1']['Pmis']:nn['iN']['Pmis']] / baseMVA
    bus[:, pidx.bus['LAM_Q']] = info['lmbda'][nn['i1']['Qmis']:nn['iN']['Qmis']] / baseMVA
