...
class OModel:
    ...
    def setup(self):
        ...
        # TODO: might need to generalize the objective parse function
        # TODO: temp solution to test power balance
        pb_expr = cp.sum(self.pg) - np.sum(self.routine.pd.v) == 0 
        self.constrs['pb'] = pb_expr

        # --- parse objective functions ---
        self.parse_obj(obj_estr=self.routine.obj.e_str,
    ...
    def parse_obj(self, obj_estr, sub_map):
        """
        ...
        if self.routine.obj.sense == 'min':
            code_obj = f'cp.Minimize({code_obj})'
        elif self.routine.obj.sense == 'max':
