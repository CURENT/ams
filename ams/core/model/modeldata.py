...
import numpy as np
from ams.core.param import (BaseParam, DataParam, IdxParam, NumParam)
...
class ModelData:
    ...
    def __init__(self, *args, three_params=True, **kwargs):
        ...
        if three_params is True:
            ...
            self.u = NumParam(
                default=1, info='connection status', unit='bool', tex_name='u')
    ...
    def find_idx(self, keys, values, allow_none=False, default=False):
        ...
        if isinstance(keys, str):
            ...
            if not isinstance(values, (int, float, str, np.floating)) and not isinstance(values, Iterable):
                raise ValueError(
                    f"value must be a string, scalar or an iterable, got {values}")
            ...
            if len(values) > 0 and not isinstance(values[0], Iterable):
                raise ValueError(
                    f"if keys is an iterable, values must be an iterable of iterables. got {values}")
        ...
        for v_search in zip(*values):
            ...
            if v_idx is None:
                if allow_none is False:
                    raise IndexError(
                        f'{list(keys)}={v_search} not found in {self.class_name}')
                else:
                    v_idx = default
