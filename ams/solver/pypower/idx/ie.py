...
def int2ext(ppc, val_or_field=None, oldval=None, ordering=None, dim=0):
    ...
    if val_or_field is None: # nargin == 1
        ...
        if o["state"] == 'i':
            ...
            ppc["bus"][o["bus"]["status"]["on"], const.bus['BUS_I']] = \
                o["bus"]["i2e"] \
                    [ ppc["bus"][o["bus"]["status"]["on"], const.bus['BUS_I']].astype(int) ]
            ppc["branch"][o["branch"]["status"]["on"], const.branch['F_BUS']] = \
                o["bus"]["i2e"][ ppc["branch"] \
                    [o["branch"]["status"]["on"], const.branch['F_BUS']].astype(int) ]
            ppc["branch"][o["branch"]["status"]["on"], const.branch['T_BUS']] = \
                o["bus"]["i2e"][ ppc["branch"] \
                    [o["branch"]["status"]["on"], const.branch['T_BUS']].astype(int) ]
            ppc["gen"][o["gen"]["status"]["on"], const.gen['GEN_BUS']] = \
                o["bus"]["i2e"][ ppc["gen"] \
                    [o["gen"]["status"]["on"], const.gen['GEN_BUS']].astype(int) ]
            if 'areas' in ppc:
                ppc["areas"][o["areas"]["status"]["on"], const.area['PRICE_REF_BUS']] = \
                    o["bus"]["i2e"][ ppc["areas"] \
                    [o["areas"]["status"]["on"], const.area['PRICE_REF_BUS']].astype(int) ]
...
def int2ext1(i2e, bus, gen, branch, areas):
    ...
    """
    bus[:, const.bus['BUS_I']]    = i2e[ bus[:, const.bus['BUS_I']].astype(int) ]
    gen[:, const.gen['GEN_BUS']]  = i2e[ gen[:, const.gen['GEN_BUS']].astype(int) ]
    branch[:, const.branch['F_BUS']] = i2e[ branch[:, const.branch['F_BUS']].astype(int) ]
    branch[:, const.branch['T_BUS']] = i2e[ branch[:, const.branch['T_BUS']].astype(int) ]

    if areas != None and len(areas) > 0:
        areas[:, const.area['PRICE_REF_BUS']] = i2e[ areas[:, const.area['PRICE_REF_BUS']].astype(int) ]
...
def ext2int(ppc, val_or_field=None, ordering=None, dim=0):
    ...
    if val_or_field is None:  # nargin == 1
        ...
        if first or ppc["order"]["state"] == 'e':
            ...
            bt = ppc["bus"][:, const.bus['BUS_TYPE']]
            err = find(~((bt == const.bus['PQ']) | (bt == const.bus['PV']) | (bt == const.bus['REF']) | (bt == const.bus['NONE'])))
            if len(err) > 0:
                logger.debug('ext2int: bus %d has an invalid BUS_TYPE\n' % err)

            ## determine which buses, branches, gens are connected and
            ## in-service
            n2i = sparse((range(nb), (ppc["bus"][:, const.bus['BUS_I']], np.zeros(nb))),
                         shape=(max(ppc["bus"][:, const.bus['BUS_I']].astype(int)) + 1, 1))
            n2i = (np.array( n2i.todense().flatten() )[0, :]).astype(int) # as 1D array
            bs = (bt != const.bus['NONE'])                               ## bus status
            o["bus"]["status"]["on"]  = find(  bs )         ## connected
            o["bus"]["status"]["off"] = find( ~bs )         ## isolated
            gs = ( (ppc["gen"][:, const.bus['GEN_STATUS']] > 0) &          ## gen status
                    bs[ n2i[ppc["gen"][:, const.gen['GEN_BUS']].astype(int)] ] )
            o["gen"]["status"]["on"]  = find(  gs )    ## on and connected
            o["gen"]["status"]["off"] = find( ~gs )    ## off or isolated
            brs = ( ppc["branch"][:, const.bus['BR_STATUS']].astype(int) &  ## branch status
                    bs[n2i[ppc["branch"][:, const.branch['F_BUS']].astype(int)]] &
                    bs[n2i[ppc["branch"][:, const.branch['T_BUS']].astype(int)]] ).astype(bool)
            o["branch"]["status"]["on"]  = find(  brs ) ## on and conn
            o["branch"]["status"]["off"] = find( ~brs )
            if 'areas' in ppc:
                ar = bs[ n2i[ppc["areas"][:, const.area['PRICE_REF_BUS']].astype(int)] ]
                o["areas"] = {"status": {}}
...
#                ppc["areas"][o["areas"]["status"]["off"], :] = array([])
            ...
            o["bus"]["i2e"] = ppc["bus"][:, const.bus['BUS_I']].copy()
            o["bus"]["e2i"] = np.zeros(max(o["bus"]["i2e"]).astype(int) + 1)
            o["bus"]["e2i"][o["bus"]["i2e"].astype(int)] = np.arange(nb)
            ppc["bus"][:, const.bus['BUS_I']] = \
                o["bus"]["e2i"][ ppc["bus"][:, const.bus['BUS_I']].astype(int) ].copy()
            ppc["gen"][:, const.gen['GEN_BUS']] = \
                o["bus"]["e2i"][ ppc["gen"][:, const.gen['GEN_BUS']].astype(int) ].copy()
            ppc["branch"][:, const.branch['F_BUS']] = \
                o["bus"]["e2i"][ ppc["branch"][:, const.branch['F_BUS']].astype(int) ].copy()
            ppc["branch"][:, const.branch['T_BUS']] = \
                o["bus"]["e2i"][ ppc["branch"][:, const.branch['T_BUS']].astype(int) ].copy()
            if 'areas' in ppc:
                ppc["areas"][:, const.area['PRICE_REF_BUS']] = \
                    o["bus"]["e2i"][ ppc["areas"][:,
                        const.area['PRICE_REF_BUS']].astype(int) ].copy()

            ## reorder gens in order of increasing bus number
            o["gen"]["e2i"] = argsort(ppc["gen"][:, const.gen['GEN_BUS']])
...
def ext2int1(bus, gen, branch, areas=None):
    ...
    @see: U{http://www.pserc.cornell.edu/matpower/}
    """
    i2e = bus[:, const.bus['BUS_I']].astype(int)
    e2i = np.zeros(max(i2e) + 1)
    e2i[i2e] = np.arange(bus.shape[0])

    bus[:, const.bus['BUS_I']]    = e2i[ bus[:, const.bus['BUS_I']].astype(int)    ]
    gen[:, const.gen['GEN_BUS']]  = e2i[ gen[:, const.gen['GEN_BUS']].astype(int)  ]
    branch[:, const.branch['F_BUS']] = e2i[ branch[:, const.branch['F_BUS']].astype(int) ]
    branch[:, const.branch['T_BUS']] = e2i[ branch[:, const.branch['T_BUS']].astype(int) ]
    if areas is not None and len(areas) > 0:
        areas[:, const.area['PRICE_REF_BUS']] = e2i[ areas[:, const.area['PRICE_REF_BUS']].astype(int) ]
