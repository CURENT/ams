...
from numpy import copy, array, ones, zeros, r_, c_, ix_,
from numpy import linalg, isscalar, dot, diag
from numpy import Inf, pi, arange, nonzero, sin, cos, arctan2
...
def makeAang(branch, nb, ppopt):
    ...
    if ignore_ang_lim:
        ...
        if nang > 0:
            ...
            jj = r_[branch[iang, const.branch['const.branch['F_BUS']']], branch[iang, const.branch['const.branch['T_BUS']']]]
...
def makeBdc(baseMVA, bus, branch):
    ...
    if any(bus[:, const.bus['BUS_I']] != list(range(nb))):
    ...
    ##
    stat = branch[:, const.branch['BR_STATUS']]               ## ones at in-service branches
    b = stat / branch[:, const.branch['BR_X']]                ## series susceptance
    ...
    f = branch[:, const.branch['const.branch['F_BUS']']]                           ## list of "from" buses
    t = branch[:, const.branch['const.branch['T_BUS']']]                           ## list of "to" buses
...
def makeLODF(branch, PTDF):
    ...
    """
    nl, nb = PTDF.shape
    f = branch[:, const.branch['const.branch['F_BUS']']]
    t = branch[:, const.branch['const.branch['T_BUS']']]
...
def makePTDF(baseMVA, bus, branch, slack=None):
    ...
    if slack is None:
        slack = find(bus[:, const.branch['BUS_TYPE']] == const.bus['REF'])
        slack = slack[0]
    ...
    if any(bus[:, const.bus['BUS_I']] != arange(nb)):
